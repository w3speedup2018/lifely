...
$('#productSliderThumbs').slick({
        ...
        if(variation_id != '' && variation_id != null && variation_id != 'undefined'){
            ...
            if(weight != null && weight != 'undefined'){              
              $.ajax({
                ...
                data: {zip:postcode,grams:weight,currency:"AUD"},
                crossDomain: true,
                dataType: 'json',
                success: function(data) { 
                  if(data.error != '' && data.error != null && data.error != 'undefiend'){
                    shipping_rate_p = 'not_found';
                    $('.csv_result_show li.shipping_deatil .shipping_fee').hide();
                  } else{
                    $('.csv_result_show li.shipping_deatil .shipping_fee').show();
                    if(data.rates[0].total_price > 0){       
                      ...
                      $('.csv_result_show li.shipping_deatil .shipping_fee').html('<span class="price_">'+'$'+price+'</span>');
                      ...
                      $('.csv_result_show li.shipping_deatil .shipping_fee').html('<span class="price_">Free</span>');
                      shipping_rate_p = 'found';
                    } 
                  }
            ...
            if(curent_variation_id != '' && curent_variation_id != null && curent_variation_id != 'undefined'){
              ...
              if(found_pre_order == 'true' && dispatch_date != '' && dispatch_date != null && dispatch_date != 'undefined'){                
                     ...
                     if(CurrentDate < today){                   
                        ...
                        $('.dispatch_check_list .dispatch_deatil .dispatch_label').html('Available On: ');
                        $('.dispatch_check_list .dispatch_deatil .dispatch_date').html(str);
                     }else{                   
                        $('.postcode_form_grid .form-note .span_dispatch_note').text('Dispatch in 24 hours');
                        $('.dispatch_check_list .dispatch_deatil .dispatch_label').html('Dispatch in: ');
                        $('.dispatch_check_list .dispatch_deatil .dispatch_date').html('24 hours');
                     }
                    $('.dispatch_check_list').show();                    
         ...
         success:function(data)
         {
           ...
           for(var count = 0; count< file_data.length; count++)
            {
              ...
              if(table_data == ''){
                ...
                $('.postcode_check_grid .posthead .deliver_address_col span').html('Delivery');
                $('.csv_result_show').hide();
                $('.postcode_check_grid .posthead').hide();   
                $('.dispatch_check_list').hide();
              }else{                
                $('form#post_check_form').hide();
                $('.post_code_error').css('display','none');	
              	$('.postcode_form_grid .post_form_head').hide();
                $('.postcode_check_grid .posthead').show();
              	//$('.postcode_form_grid .form-inner-wrap').hide();
             	//$('#csv-display').fadeIn();
            	if(found_pre_order == 'true'){
                      $('.csv_result_show li.delivery_detail span.delivery_date').html('Not Eligible');
                }else{
                  	$('#csv-display .exect_date').html(table_data);
                	$('.csv_result_show li.delivery_detail span.delivery_date').html(table_data);
                }              
                $('div#csv-display h3.diliverd_to span:first-child').html(address);
                $('.postcode_check_grid .posthead .deliver_address_col span').html(address);
           ...
           if(table_data == ''){             		
                if(shipping_rate_p == 'found'){
                     ...
                     $('.postcode_check_grid .posthead .deliver_address_col span').html(postcode);
                     $('form#post_check_form').hide();                  
                     $('.postcode_check_grid .posthead').show();
                     $('.csv_result_show').show();
                     $('.dispatch_check_list').show();
 ...
 $(document).on('mouseleave','.collection_product_color_swatches li',function() {
  ...
  $('.Recheck_Shipping_button').click(function(e){
    e.preventDefault();
    $('.mattress-pdp-new-design .postcode_check_grid .posthead').hide();
    $('.mattress-pdp-new-design ul.csv_result_show').hide();
    $('.mattress-pdp-new-design .post_check_main_grid .postcode_check_grid form#post_check_form').show();
  });
 
  var curent_request = null;
  var curent_date_request = null;

  $('.mattress-pdp-new-design variant-selects.no-js-hidden .select select').change(function(){
    setTimeout(function(){
      var variation_id = $('product-form.product-form input[name="id"]').attr('value');
  ...
  //$('product-form.product-form').on('DOMSubtreeModified',function(){
  $('.mattress-pdp-new-design form[data-type="add-to-cart-form"] input[name="id"]').change(function(){
    var postcode = $('#post_check_form #post_check').val();
    var variation_id = $('product-form.product-form input[name="id"]').attr('value'); 
    if($(this).val() != '' && $(this).val() != 'undefiend' && $(this).val() != null){
      $('.matress_pdp_mobile_4_payment .product_payement_logo_new.active').removeClass('active');
      $('.matress_pdp_mobile_4_payment .product_payement_logo_new.product_payement_logo_'+variation_id).addClass('active');
      $('.preorder_discout_bar p').hide();  
      if(variation_id != '' && variation_id != null && variation_id != 'undefined'){
  		var stock_label = $('.InventoryManagment_sec_'+variation_id+' span').attr('data-stock-type');
  		var instock_label = $('.InventoryManagment_sec_'+variation_id+' span').html();
  		var preorder_batch_c = $('#productSliderGallery .preorder_batch').length;    
  ...
  $('.product__info-container a.viewmore-details').click(function(e){
   ...
   if($('.prder_order_listing.simple_product_pre_order').length){
      if($('#productSliderGallery .preorder_batch').length == 0){
          $('#productSliderGallery').append('<span class="preorder_batch">Pre-order</span>');
      }
   }
   setTimeout(function(){
  ...
  $('.product_info_video .deep_dream_video_play_icon span').click(function(e){
    e.preventDefault();
    $(this).closest('.product_info_video').find('video').get(0).play();
    $(this).closest('.deep_dream_video_play_icon').hide();
  });

  $('.product_info_video video').each(function(){
    $(this).on('ended', function(){
      $(this).closest('.product_info_video').find('.deep_dream_video_play_icon').show();
    });
  });
});
